...
async def _extract_audio(video_path: str, output_path: str | None = None) -> str:
    ...
    try:
        await run_async(cmd, timeout=60, text=True, check=True)
...
async def _measure_loudness_ffmpeg(audio_path: str) -> LoudnessMetrics:
    ...
    try:
        _rc, _, stderr = await run_async(cmd, timeout=60, text=True)
    except (RuntimeError, FileNotFoundError) as exc:
...
async def _detect_silence(audio_path: str, threshold_db: float = -40.0, min_duration: float = 2.0) -> list[dict[str, float]]:
...
async def _detect_clipping(audio_path: str) -> dict[str, Any]:
...
async def _get_audio_duration(audio_path: str) -> float:
    ...
    try:
        _rc, stdout, _ = await run_async(cmd, timeout=30, text=True)
        raw = stdout.strip()
...
class AudioReviewAgent(BaseAudioReviewAgent):
    ...
    async def review_audio(
        ...
        if audio_path.endswith((".mp4", ".mkv", ".mov", ".avi", ".webm")):
            try:
                extracted_audio_path = await _extract_audio(audio_path)
        ...
        loudness_metrics = await _measure_loudness_ffmpeg(source_path)
    ...
    async def _run_omni_review(
        ...
        if audio_path.endswith((".mp4", ".mkv", ".mov", ".avi", ".webm")):
            try:
                extracted_for_analysis = await _extract_audio(audio_path)
                analysis_path = extracted_for_analysis
            except RuntimeError:
                analysis_path = audio_path  # try anyway

        duration = await _get_audio_duration(analysis_path)
        silence_segments = await _detect_silence(analysis_path)
        clipping_info = await _detect_clipping(analysis_path)
    ...
    async def measure_loudness(
        ...
        source_path = audio_path
        if audio_path.endswith((".mp4", ".mkv", ".mov", ".avi", ".webm")):
            source_path = await _extract_audio(audio_path)

        metrics = await _measure_loudness_ffmpeg(source_path)
        return {
